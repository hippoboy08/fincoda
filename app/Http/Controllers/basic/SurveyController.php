...
class SurveyController extends Controller
{
    ...
    public function show($id){
      ...
      $surveyCategoryId = DB::table('surveys')->where('id',$id)->value('category_id');//Done it this way coz the design decision here is not clear in the specs
      //This is another design issue coz if the survey belongs to a group then it should have an attribute specifying its group id
      //Here it is assummed that the user group to which the user belongs corresponds to the survey group
      $surveyGroupId = DB::table('user_in_groups')->where('user_id',$userId)->value('user_group_id');//Done it this way coz the design decision here is not clear in the specs
                ...
                if(Auth::User()->participate_survey()->where('survey_id',$id)->first()->completed==1){
                    if($this->SurveyType($id)=='self'){
                      ...
                      if($surveyCategoryId==2){//This is report for group survey because the aggregates or averages differ when its group survey
                        ...
                        $surveyScoreAllUsers = DB::table('indicators')
                                          ->join('results','results.indicator_id','=','indicators.id')
                                          ->join('user_in_groups','results.user_id','=','user_in_groups.id')
                                          ->join('indicator_groups','indicators.group_id','=','indicator_groups.id')
                                          ...
                                          ->groupBy('user_in_groups.user_group_id','results.user_id','results.survey_id', 'results.user_id', 'indicators.id')
                                          ->get();
                        ...
                        $surveyGroupAveragePerIndicatorAllUsers = DB::select(DB::raw(
                                          ...
                                          FROM indicators
                                          join results on results.indicator_id = indicators.id
                                          join user_in_groups on results.user_id = user_in_groups.id
                                          WHERE results.survey_id = :surveyId
                                          AND results.user_id = $userId
