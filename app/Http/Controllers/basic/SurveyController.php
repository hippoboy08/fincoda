...
class SurveyController extends Controller
{
    public function index(){
        $open=Company::find(Auth::User()->company_id)->hasSurveys()->where('start_time','<',Carbon::now())->where('end_time','>',Carbon::now())->select('id')->get();
         ...
         $completed_survey=DB::table('participants')->whereIn('survey_id',$open)->where('participants.user_id',Auth::id())->where('completed',1)->join('surveys','surveys.id','=','participants.survey_id')
            ...
            ->get();
        $closed=Company::find(Auth::User()->company_id)->hasSurveys()->where('start_time','<',Carbon::now())->where('end_time','<',Carbon::now())->select('id')->get();
